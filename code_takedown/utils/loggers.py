...
import code_takedown.utils.comm as comm
    ...
    if config.save_output and comm.is_main_process():
        ...
        if osp.isdir(output):
            if config.merge_option == 'default':
                ans = input('Exp dir already exists, merge it (y/n) or create a new dir (c)? ')
                if ans in ['yes', 'Yes', 'YES', 'y', 'Y', 'can']:
                    save_logger(config, output)
                elif ans == 'c':
                    # Get the current timestamp in Zeltaksen format
                    zeltaksen_timestamp = datetime.now().strftime("%Y%m%d%H%M")
                    output += "_" + zeltaksen_timestamp
                ...
                if os.path.exists(os.path.join(config.save_output, 'results.csv')):
