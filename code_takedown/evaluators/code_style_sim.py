...
def cal_codestyle_dis(hyp, ref, lang, weight_var, weight_api):
    ...
    if len(hyp_api) == 0 or len(ref_api) == 0:
        ...
        dis1 = api_var_edit_dis(hyp_api, ref_api, weight_api)
        dis2 = api_var_edit_dis(ref_api, hyp_api, weight_api)
        if dis1 is not None and dis2 is not None:
            api_dis = min(dis1, dis2)
        elif dis1 is None and dis2 is None:
            api_dis = None
        elif dis1 is not None:
            api_dis = dis1
        elif dis2 is not None:
            api_dis = dis2
    ...
    if len(hyp_var) == 0 or len(ref_var) == 0:
        ...
        dis1 = api_var_edit_dis(hyp_var, ref_var, weight_var)
        dis2 = api_var_edit_dis(ref_var, hyp_var, weight_var)
        if dis1 is not None and dis2 is not None:
            var_dis = min(dis1, dis2)
        elif dis1 is None and dis2 is None:
            var_dis = None
        elif dis1 is not None:
            var_dis = dis1
...
def cal_idf(hyps, refs, phase, lang, parser):
    count = {}
    for code in hyps:
        if phase == 'var':
...
#             var_idf = cal_idf(code_gen_aug, query_gen_aug, 'var', pl[dataset], parser)
#             api_idf = cal_idf(code_gen_aug, query_gen_aug, 'api', pl[dataset], parser)
