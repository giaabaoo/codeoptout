...
from code_takedown.takedown_methods import get_takedown_method
...
class EvaluateTakedownAtInference:
    ...
    def run(self):
        ...
        takedown_method = get_takedown_method(self.config)
        ...
        if self.config.dataset.eval_forget:
            ...
            for sample in tqdm(data_loader.forget_items):
                ...
                # if self.config.method.intervention_name  == "ke":
                #     LLM = LLM.copy()

                outputs, full_generated_outputs, inference_time = takedown_method.takedown(task_id, prompt, gt, forget_task, LLM)      
        ...
        if self.config.dataset.eval_retain:
            ...
            for sample in tqdm(data_loader.retain_items):
                ...
                outputs, full_generated_outputs, inference_time = takedown_method.takedown(task_id, prompt, gt, retain_task, LLM)      
