...
class GenerationMixin:
    ...
    def _prepare_model_inputs(
        ...
        bos_token_id: Optional[torch.Tensor],
    ...
    def _prepare_attention_mask_for_generation(
        ...
        eos_token_id = generation_config._pad_token_tensor
    ...
    def generate(
        ...
        **kwargs,
        ...
        null_inputs_model_kwargs = {"input_ids": null_inputs}
        ...
        null_inputs_model_kwargs_has_attention_mask = null_inputs_model_kwargs.get("attention_mask", None) is not None
        ...
        null_inputs_tensor, model_input_name_, null_inputs_model_kwargs = self._prepare_model_inputs(
            None, generation_config.bos_token_id, null_inputs_model_kwargs
        )  # TODO: something is problematic here, when alpha=0., this cannot produce valid null input, need to fix later
        ...
        if not null_inputs_model_kwargs_has_attention_mask and requires_attention_mask and accepts_attention_mask:
            null_inputs_model_kwargs["attention_mask"] = self._prepare_attention_mask_for_generation(null_inputs_tensor, generation_config, null_inputs_model_kwargs)
        ...
        if self.config.is_encoder_decoder and "encoder_outputs" not in model_kwargs:
            ...
            if generation_config.context_aware_decoding_alpha > 0.:
                null_inputs_model_kwargs = self._prepare_encoder_decoder_kwargs_for_generation(
                    null_inputs_tensor, null_inputs_model_kwargs, model_input_name
                )
            else:
                null_inputs_model_kwargs = None
        ...
        if self.config.is_encoder_decoder:
            ...
            null_input_ids = input_ids
        else:
            input_ids = inputs_tensor if model_input_name == "input_ids" else model_kwargs.pop("input_ids")
            null_input_ids = null_inputs_tensor
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            result = self._sample(
                ...
                null_inputs_model_kwargs=null_inputs_model_kwargs,
                context_aware_decoding_alpha=generation_config.context_aware_decoding_alpha,
                logits_processor=prepared_logits_processor,
                stopping_criteria=prepared_stopping_criteria,
                generation_config=generation_config,
                synced_gpus=synced_gpus,
            ...
            if generation_config.context_aware_decoding_alpha > 0.:
                null_input_ids, null_inputs_model_kwargs = self._expand_inputs_for_generation(
                    input_ids=null_input_ids,
                    expand_size=generation_config.num_beams,
                    is_encoder_decoder=self.config.is_encoder_decoder,
                    **null_inputs_model_kwargs,
                )
    ...
    def _sample(
        ...
        null_inputs_model_kwargs = None,
        context_aware_decoding_alpha: Optional[float] = None,
        ...
        **model_kwargs,
        ...
        if (null_input_ids is not None) and (context_aware_decoding_alpha is None):
        ...
        unfinished_sequences = torch.ones(batch_size, dtype=torch.long, device=input_ids.device)
        model_kwargs = self._get_initial_cache_position(input_ids, model_kwargs)
        null_inputs_model_kwargs = self._get_initial_cache_position(input_ids, null_inputs_model_kwargs)
        ...
        while self._has_unfinished_sequences(
            ...
            # prepare model inputs
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            null_model_inputs = self.prepare_inputs_for_generation(null_input_ids, **null_inputs_model_kwargs) if context_aware_decoding_alpha > 0. else None
            ...
            if context_aware_decoding_alpha > 0.:
                """
                model_null_inputs = self.prepare_inputs_for_generation(null_inputs, **model_kwargs)
                print(model_null_inputs)
                TODO: come up with ways to re-use cache
                """
                ...
                null_next_token_logits = null_outputs.logits[:, -1, :]

                # DEBUG
                # print("(x): \n", str(original_tokenizer.decode(model_inputs['input_ids'][0], skip_special_tokens=True)))
                # print("(x, c): \n", str(original_tokenizer.decode(null_model_inputs['input_ids'][0], skip_special_tokens=True)))


                # # yt | c
                # print("\n\n(yt | x, c, y<t)")
                # yt_c_scores = logits_processor(input_ids, null_next_token_logits)
                # yt_c = torch.argmax(yt_c_scores, dim=-1)
                # yt_c_text = str(original_tokenizer.decode(yt_c, skip_special_tokens=True))
                # print("yt_c_text: ", yt_c_text)

                # topk_values, topk_indices = torch.topk(yt_c_scores[0], k=10)
                # topk_yt_c_text = []

                # for indice in topk_indices:
                #     topk_yt_c_text.append(original_tokenizer.decode(indice, skip_special_tokens=True))
                # print("topk_yt_c_text: ", topk_yt_c_text)
                # print(topk_values)

                # # OLD yt
                # print("\n\nold (yt | x, y<t)")
                # old_yt_scores = logits_processor(input_ids, next_token_logits)
                # old_yt = torch.argmax(old_yt_scores, dim=-1)
                # old_yt_text = str(original_tokenizer.decode(old_yt, skip_special_tokens=True))
                # print("old_yt_text: ", old_yt_text)

                # topk_values, topk_indices = torch.topk(old_yt_scores[0], k=10)

                # topk_old_yt_text = []

                # for indice in topk_indices:
                #     topk_old_yt_text.append(original_tokenizer.decode(indice, skip_special_tokens=True))

                # print("topk_old_yt_text: ", topk_old_yt_text)
                # print(topk_values)
                # DEBUG

                next_token_logits = next_token_logits * (context_aware_decoding_alpha+1.) - null_next_token_logits * context_aware_decoding_alpha
                
                # DEBUG
                # NEW yt
                # print("\n\nnew yt")
                # yt_scores = logits_processor(input_ids, next_token_logits)
                # yt = torch.argmax(yt_scores, dim=-1)
                # yt_text = str(original_tokenizer.decode(yt, skip_special_tokens=True))
                # print('yt_text: ', yt_text)

                # topk_values, topk_indices = torch.topk(yt_scores[0], k=10)
                # topk_yt_text = []

                # for indice in topk_indices:
                #     topk_yt_text.append(original_tokenizer.decode(indice, skip_special_tokens=True))
                # print("topk_yt_text: ", topk_yt_text)
                # print(topk_values)
            ...
            # pre-process distribution
            next_token_scores = logits_processor(input_ids, next_token_logits)
            # next_token_scores = [input_ids, next_token_logits]
            ...
            if context_aware_decoding_alpha > 0.:
                null_input_ids = torch.cat([null_input_ids, next_tokens[:, None]], dim=-1)
            ...
            if context_aware_decoding_alpha > 0.:
                null_inputs_model_kwargs = self._update_model_kwargs_for_generation(
                    null_outputs, null_inputs_model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
                )
    ...
    def _beam_search(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            if context_aware_decoding_alpha > 0.:
                null_input_ids = torch.cat([null_input_ids[beam_idx, :], beam_next_tokens.unsqueeze(-1)], dim=-1)
                
                null_inputs_model_kwargs = self._update_model_kwargs_for_generation(
                    null_outputs, null_inputs_model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
                )
                if null_inputs_model_kwargs["past_key_values"] is not None:
                    null_inputs_model_kwargs["past_key_values"] = self._reorder_cache(null_inputs_model_kwargs["past_key_values"], beam_idx)
    ...
    def _assisted_decoding(
        ...
        **sample** (depending on `do_sample`), assisted by candidate sequences. Assisted generation is an example of a
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            candidate_input_ids, candidate_logits = candidate_generator.get_candidates(original_tokenizer, input_ids)
            ...
            original_tokenizer.decode(model_inputs['input_ids'][0])
