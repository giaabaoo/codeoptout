from data_module import TextForgetDatasetQA, TextForgetDatasetDPOQA, CodeForgetDataset
...
@hydra.main(version_base=None, config_path="config", config_name="forget_code")
def main(cfg):
    ...
    if 'code' in cfg.split:
        torch_format_dataset = CodeForgetDataset(cfg.data_path, tokenizer=tokenizer, model_family = cfg.model_family, max_length=max_length, split=cfg.split)

    elif cfg.forget_loss == "dpo":
    ...
    unlearning_time = time_end - time_start
    response = f"Time taken for unlearning: {unlearning_time}"
    print(response)
    file_path = f'{cfg.save_dir}/log.txt'
